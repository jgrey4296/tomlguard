...
class TestFailureProxy:
    ...
    def test_initial(self):
        proxy = TomlGuardFailureProxy(None, fallback=2)
    ...
    def test_call_get_value(self):
        proxy = TomlGuardFailureProxy(5, fallback=2)
        assert(proxy() == 5)

    def test_call_get_None_value(self):
        proxy = TomlGuardFailureProxy(None, fallback=None)
        assert(proxy() == None)
    ...
    def test_wrapper(self):
        proxy = TomlGuardFailureProxy(3, fallback=2)
    ...
    def test_types(self):
        proxy = TomlGuardFailureProxy(True, fallback=2, types=int)
        assert(proxy is not None)

    def test_types_fail(self):
        with pytest.raises(TypeError):
            TomlGuardFailureProxy(None, fallback="blah", types=int)

    def test_proxy_inject(self):
        proxy1 = TomlGuardFailureProxy(None, fallback=2, types=int)
    ...
    def test_proxy_inject_index_update(self):
        proxy1 = TomlGuardFailureProxy(None, fallback=2, types=int).blah.bloo
